"""This is an alternative version of the main script to run the evaluation experiment on the Monaco dataset.
In this version we limit the number of concurrent calls to the LLM as Ragas does not have a way to limit the
number of concurrent calls to the LLM and this can lead to problems with hitting rate limits.
See Ragas issue: https://github.com/vibrantlabsai/ragas/issues/2457 still open as of 08/2026
"""
# %% library imports
import json
...
# %% stop too much multi-threading leading to Azure API time out (429 error)
# this is code generated by Claude Sonnet 4.6 to fix too many concurrent calls
# this is a workaround as Ragas has an open issue about controlling this
# and python bugs shows suggestions to control this directly in asyncio
# Also useful to avoid trying to instantiate too many local embedding models in parallel
# or passing too many queries to the neo4j db at once
...
GRAPH_INVOKE_TIMEOUT = 900  # seconds before a single graph.invoke is considered hung

async def run_sync_limited(func, *args, **kwargs):
...
async def run_async_limited(coro):
    async with LLM_SEMAPHORE:
        return await coro
    ...
    async def my_experiment(row):
        ...
        if response_dict: # don't attempt to evaluate if there was no response generated
            ...
            try:
                ...
                factual_correctness_precision.ascore(
            ...
            try:
                ...
                factual_correctness_recall.ascore(
                        response=str(agent_answer), # requires string not list
                        reference=row["expected_answer"]
                    )
                )
            ...
            try:
                crag_result = await run_async_limited(
                    crag_metric.ascore(
                        llm=evaluator_llm,
...
async def main():
    ...
    for file in dataset_csv_files: # slice here to limit how many batches for testing, we only used [:16] for final eval due to time/cost constraints
        ...
        # Restart thread pool for this batch to clear any stuck threads
        if THREAD_POOL is not None:
            print(f"Shutting down previous thread pool...")
            THREAD_POOL.shutdown(wait=True)
            await asyncio.sleep(0.2)  # Brief pause for OS cleanup
        
        THREAD_POOL = ThreadPoolExecutor(max_workers=MAX_CONCURRENCY)
        print(f"Started fresh thread pool for batch {batch_num}")
...
if __name__ == "__main__":
    try:
        ...
        # Enforce shutdown with timeout to prevent hanging on stuck tasks
        ...
        shutdown_thread = threading.Thread(target=shutdown_thread_pool, daemon=True)
        shutdown_thread.start()
        shutdown_thread.join(timeout=60)  # Wait max 60 seconds
        
        if shutdown_thread.is_alive():
            # Shutdown is taking too long, force it
            print("Thread pool shutdown timed out after 60s, forcing shutdown...")
